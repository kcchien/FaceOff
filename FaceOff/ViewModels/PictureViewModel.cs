...
namespace FaceOff
{
	public class PictureViewModel : BaseViewModel
	{
		...
		readonly string[] _emotionStringsForAlertMessage = { "怒", "不屑", "討厭", "害怕", "開心", "淡定", "傷心", "驚喜" };

		const string _makeAFaceAlertMessage = "拍一張最能表達";
		const string _calculatingScoreMessage = "人工智慧分析中...";
		...
		{
			{ EmotionType.Anger, "怒！" },
			{ EmotionType.Contempt, "不屑" },
			{ EmotionType.Disgust, "討厭"},
			{ EmotionType.Fear, "害怕" },
			{ EmotionType.Happiness, "開心" },
			{ EmotionType.Neutral, "淡定" },
			{ EmotionType.Sadness, "傷心" },
			{ EmotionType.Surprise, "驚喜" }
		};
		#endregion
		...
		public void SetPhotoImage1ToHappyForUITest(string photo1ImageString)
		{
			...
			allEmotionsStringBuilder.AppendLine($"怒: 0%");
			allEmotionsStringBuilder.AppendLine($"不屑: 0%");
			allEmotionsStringBuilder.AppendLine($"討厭: 0%");
			allEmotionsStringBuilder.AppendLine($"害怕: 0%");
			allEmotionsStringBuilder.AppendLine($"開心: 100%");
			allEmotionsStringBuilder.AppendLine($"淡定: 0%");
			allEmotionsStringBuilder.AppendLine($"傷心: 0%");
			allEmotionsStringBuilder.AppendLine($"驚喜: 0%");

			_photo1Results = allEmotionsStringBuilder.ToString();
			ScoreButton1Text = "得分: 100%";
		...
		async Task ExecuteTakePhoto(PlayerModel playerModel)
		{
			...
			if (doesEmotionScoreContainErrorMessage)
			{
				...
				SetScoreButtonText($"得分: {emotionScore}", playerModel.PlayerNumber);
		...
		string GetStringOfAllPhotoEmotionScores(Emotion[] emotionResults, int emotionResultNumber)
		{
			...
			allEmotionsString.AppendLine($"怒: {ConvertFloatToPercentage(emotionResults[emotionResultNumber].Scores.Anger)}");
			allEmotionsString.AppendLine($"不屑: {ConvertFloatToPercentage(emotionResults[emotionResultNumber].Scores.Contempt)}");
			allEmotionsString.AppendLine($"討厭: {ConvertFloatToPercentage(emotionResults[emotionResultNumber].Scores.Disgust)}");
			allEmotionsString.AppendLine($"害怕: {ConvertFloatToPercentage(emotionResults[emotionResultNumber].Scores.Fear)}");
			allEmotionsString.AppendLine($"開心: {ConvertFloatToPercentage(emotionResults[emotionResultNumber].Scores.Happiness)}");
			allEmotionsString.AppendLine($"淡定: {ConvertFloatToPercentage(emotionResults[emotionResultNumber].Scores.Neutral)}");
			allEmotionsString.AppendLine($"傷心: {ConvertFloatToPercentage(emotionResults[emotionResultNumber].Scores.Sadness)}");
			allEmotionsString.Append($"驚喜: {ConvertFloatToPercentage(emotionResults[emotionResultNumber].Scores.Surprise)}");
		...
		async Task<bool> DisplayPopUpAlertAboutEmotion(string playerName)
		{
			...
			{
				Title = "來比" + _emotionDictionary[_currentEmotionType],
				Message = playerName + ", " + _makeAFaceAlertMessage + _emotionStringsForAlertMessage[(int)_currentEmotionType] + "的照片吧！"
			};
